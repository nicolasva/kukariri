  ...
  initialize: (options) ->
    $(window).bind 'hashchange', ->
        ...
        if regex.test(window_location_hash) == true
          ...
          $($(".header").children().first().children()[1]).children().first().attr("href", "#/items/#{item_id}/types/#{type_id}/contacts/new")
        else
          $($(".header").children().first().children()[1]).children().first().attr("href", "#/contacts/new")
